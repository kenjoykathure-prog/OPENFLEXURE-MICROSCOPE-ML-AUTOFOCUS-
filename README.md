# OPENFLEXURE MICROSCOPE-ML-AUTOFOCUS-
...
- **Support Vector Regression (SVR, RBF kernel)** — test R² = 0.970, MAE = 224.87 motor steps (11.24 µm)
...
Both models completed autofocus in **under 5 seconds** on stained, unstained, and cancer-tissue samples across 500–1500 motor-step defocus offsets, versus 29–118 seconds for the custom exhaustive search. Full per-condition breakdowns (VoL, execution time, CV, relative error, CPU/RAM) for all four methods across all three sample types are tabulated in `OFM SUPPLEMENTARY.pdf` / ` OFM SUPPLEMENTARY.md` and generated by the `plotting/` scripts into `results/`.
